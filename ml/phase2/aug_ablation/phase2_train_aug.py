"""
FORGE phase 2e — augmentation ablation. Runs on Kaggle (GPU or CPU fallback).

Same setup as phase2_train.py (224px, RGB-only), but training images get
random Gaussian blur and random JPEG re-compression, each applied
independently at 50% probability per image — the exact recipe from Wang et
al., CVPR 2020 ("CNN-generated images are surprisingly easy to spot...for
now"), which found a detector trained on ONE GAN generalized to many unseen
GANs specifically because this augmentation stops the model from leaning on
crisp, generator-specific pixel noise it can't get away with keeping once
that noise is randomly destroyed half the time.

The baseline (no augmentation) scored 0.000 on held-out StyleGAN3 faces.
This is the test of whether that fixes it — measured, not assumed.
"""
import csv
import random
import time
...
def random_blur_jpeg(im):
    if random.random() < 0.5:
        im = im.filter(ImageFilter.GaussianBlur(radius=random.uniform(0.5, 3.0)))
    if random.random() < 0.5:
        buf = BytesIO()
        im.save(buf, format="JPEG", quality=random.randint(30, 95))
        buf.seek(0)
...
def main():
    ...
    best_val_acc = 0.0
    log_lines = [f"device={DEVICE}, aug=blur+jpeg@0.5each, img_size={IMG_SIZE}, train={len(train_rows)} (real={n_real} fake={n_fake}, pos_weight={pos_weight.item():.3f}), val={len(val_rows)}, test={len(test_rows)}, heldout={len(heldout_rows)}\n"]
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), OUT / "model_best_aug.pt")

    model.load_state_dict(torch.load(OUT / "model_best_aug.pt"))
    ...
    report = ["# FORGE phase 2e — augmentation ablation report\n\n", "".join(f"    {l}" for l in log_lines), "\n"]
    ...
    with open(OUT / "PHASE2_AUG_REPORT.md", "w") as f:
        f.writelines(report)
    print("".join(report))
