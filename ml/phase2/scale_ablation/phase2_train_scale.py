"""
FORGE phase 2h -- data-scale ablation. Identical training recipe to
phase2_train.py (same model, same 224px/3-epoch config) -- the only thing
that changed is upstream, in the dataset: forge-normalized-v1 now includes
~10,000 StyleGAN faces (xhlulu/140k-real-and-fake-faces) instead of the
~2,500 from the first StyleGAN addition. Tests whether more volume of the
same generator lineage pushes StyleGAN3 held-out accuracy past 0.365.
...
def main():
    ...
    report = ["# FORGE phase 2h -- data-scale ablation report\n\n", "".join(f"    {l}" for l in log_lines), "\n"]
